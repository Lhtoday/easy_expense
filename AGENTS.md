...
- 默认采用 loop engineer 协作方式：用户给出目标后，Codex 应主动完成开发、必要自测、问题修复、提交和推送，再通知用户验收效果。
- 不因小实现细节频繁打断用户；实现方案应优先遵循现有代码结构、业务规则、财务合规和审计要求。
- 推送前应尽量完成与改动风险匹配的验证，包括单元测试、构建检查、接口验证、页面手工验证或本地运行检查。
- 用户验收后如提出问题或疑问，Codex 应进入下一轮闭环：定位、修改、自测、提交、推送，并再次交给用户验证。
- 仅在需求涉及业务规则取舍、高风险或破坏性操作、生产或共享部署、凭据和权限授权、或本地环境阻塞导致无法继续时，才向用户确认。
