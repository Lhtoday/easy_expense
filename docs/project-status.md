...
- 2026-07-07：Phase 9 页面流程验收通过。已补充 `scripts/seed-phase9-accounts.sql` 生成基础会计科目和默认映射，本地写入 11 个会计科目和 11 条映射；修复凭证/会计设置列表查询参数数字转换问题，解决会计设置页面空表问题。用户已反馈凭证流程可跑通。
- 2026-07-08：记录 Phase 9 后续问题：当前已开发凭证生成功能，但凭证相关状态不能回退。后续需要补齐草稿撤销、确认前回退、确认后作废/冲销或重生成策略，并同步权限、审计、状态日志和数据影响边界。

## 2026-07-16 Phase 9 收尾记录

- 新增凭证草稿整单撤销能力：`VOUCHER_DRAFTED -> PAID`，仅允许未确认草稿撤销。
- 撤销时将 `gl_vouchers` 草稿置为 `VOIDED`，写入 `gl_voucher_logs`、`exp_report_logs` 和 `sys_audit_logs`，并释放顶层付款凭证唯一约束以支持重新生成。
...
- 2026-07-16 建议 1：基础资料在刚创建或尚未被业务数据引用时，应允许修改和物理删除；一旦被报销单、审批、预算、付款、凭证、科目映射等业务记录引用，就应禁止物理删除，改为停用/禁用，并在前端显示引用原因和处理建议。后续实现时需要为部门、成本中心、项目、费用类型、预算、会计科目、科目映射等主数据补引用计数、删除前校验和清晰错误提示。
- 2026-07-17 建议 1 首批已实现：部门、成本中心、项目、费用类型、预算、会计科目和科目映射的删除入口已改为“未引用物理删除、已引用停用保留”。其中科目映射没有业务外键直接引用，已生成凭证保留独立分录快照，因此允许物理删除；用户和角色删除规则暂未纳入本次范围，后续应单独评估登录、权限分配和审计影响。
- 2026-07-21 建议 2：出纳付款阶段当前只支持“付款”和“失败”两个动作；“失败”仅记录失败付款并保持单据在 `FINANCE_APPROVED`，允许再次付款。后续建议补充“退回财务审核”和“暂挂/待处理”等异常处理动作，用于收款账户、付款信息、金额、预算或附件在出纳阶段发现问题时回退修正，而不是只能登记付款失败。暂不修改代码，作为后续付款流程优化项跟进。
